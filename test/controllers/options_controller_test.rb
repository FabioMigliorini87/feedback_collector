...
class OptionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create option" do
    assert_difference('Option.count') do
      post options_url, params: { option: { code: @option.code, response: @option.response } }
    end
  ...
  test "should update option" do
    patch option_url(@option), params: { option: { code: @option.code, response: @option.response } }
